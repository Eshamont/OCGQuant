    ...
    def forward(self, hidden_states):
        if self.fuse_rmsnorm_reorder:
            ...
            if result is not None:
                return result
        return self.originalNorm(hidden_states)
...
class QQwen3Attention(nn.Module):
    def __init__(
        ...
        self.hidden_size = getattr(self.config, "hidden_size")
        self.num_heads = self.config.num_attention_heads
        self.head_dim = getattr(originalAttn, "head_dim", getattr(self.config, "head_dim", self.hidden_size // self.num_heads))
        ...
        self.scaling = getattr(originalAttn, "scaling", self.head_dim**-0.5)
        self.attention_dropout = getattr(originalAttn, "attention_dropout", self.config.attention_dropout)
        self.sliding_window = getattr(originalAttn, "sliding_window", None)
    ...
    def forward(
        ...
        **kwargs,
        ...
        attn_output = attn_output.reshape(bsz, q_len, self.attn_output_size)
