...
#define CASE_REORDER_X_16(VAL) \
    case VAL: \
        run_reorder_x_bf16_nvfp4<16, VAL>( \
            ptr_X, M, ptr_idx, ptr_base, ptr_flags, \
            ptr_QX, ptr_SFX, \
            KQ, KE \
        ); \
        break;
...
#define CASE_REORDER_W_16(VAL) \
    case VAL: \
        run_reorder_w_bf16_nvfp4<16, VAL>( \
            ptr_W, N, ptr_idx, ptr_base, ptr_flags, \
            ptr_QW, ptr_SFW, \
            KQ, KE \
        ); \
        break;
...
)
{
    ...
    int16_t *ptr_base = nullptr;
    uint8_t *ptr_flags = nullptr;
...
std::tuple<torch::Tensor, torch::Tensor> reorder_quantize_x_meta(
        const torch::Tensor &X,
        const torch::Tensor &reorder_index,
        const torch::Tensor &reorder_base_index,
        const torch::Tensor &reorder_is_contiguous,
        const int KE
)
{
    int M = X.size(0);
    ...
    auto ptr_X   = (cutlass::bfloat16_t *)X.data_ptr<at::BFloat16>();
    auto ptr_idx = reorder_index.data_ptr<int16_t>();
    auto ptr_base = reorder_base_index.data_ptr<int16_t>();
    auto ptr_flags = reorder_is_contiguous.data_ptr<uint8_t>();
    auto ptr_QX  = QX.data_ptr<uint8_t>();
    auto ptr_SFX = reinterpret_cast<cutlass::float_ue4m3_t *>(SFX.data_ptr<uint8_t>());
...
std::tuple<torch::Tensor, torch::Tensor> reorder_quantize_w_meta(
        const torch::Tensor &W,
        const torch::Tensor &reorder_index,
        const torch::Tensor &reorder_base_index,
        const torch::Tensor &reorder_is_contiguous,
...
std::tuple<torch::Tensor, torch::Tensor> rmsnorm_quantize_x_meta(
        const torch::Tensor &X,
        const torch::Tensor &W,
        const float eps,
        const torch::Tensor &reorder_index,
...
)
{
    ...
    m.def("reorder_quantize_x_meta", &reorder_quantize_x_meta,
          "Reorder and quantize activation with group metadata",
          py::arg("X"), py::arg("reorder_index"),
          py::arg("reorder_base_index"), py::arg("reorder_is_contiguous"),
          py::arg("KE")
        );
    ...
    m.def("reorder_quantize_w_meta", &reorder_quantize_w_meta,
          "Reorder and quantize weight with group metadata",
          py::arg("W"), py::arg("reorder_index"),
          py::arg("reorder_base_index"), py::arg("reorder_is_contiguous"),
          py::arg("KE")
        );
    m.def("rmsnorm_quantize_x", &rmsnorm_quantize_x,
          "Rmsnorm and quantize activation, return K + KE channels",
	          ...
	          py::arg("KE"),
	          py::arg("global_scale") = 1.0
	        );
    m.def("rmsnorm_quantize_x_meta", &rmsnorm_quantize_x_meta,
          "Rmsnorm and quantize activation with group metadata",
          py::arg("X"), py::arg("W"), py::arg("eps"),
          py::arg("reorder_index"),
          py::arg("reorder_base_index"), py::arg("reorder_is_contiguous"),
          py::arg("KE"),
