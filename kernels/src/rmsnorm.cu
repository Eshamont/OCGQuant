...
#pragma unroll
  ...
  if (contiguous_segment) {
    #pragma unroll 4
    for(int i = 0; i < elements_per_thread; ++i){
      int reorder_idx = base_index + i;
      bf16_t input_value = input_smem[reorder_idx];
      bf16_t weight_value = weight[reorder_idx];
      bf16_t normalized_value = Float2Bfloat16((float)input_value * (float)sumv);
      bf16_t weighted_value = Float2Bfloat16((float)weight_value * (float)normalized_value);
      input_frag[i] = Float2Bfloat16((float)weighted_value / global_scale);
    }
  } else {
    #pragma unroll 4
    for(int i = 0; i < elements_per_thread; ++i){
      int reorder_idx = reorder_frag[i];
      bf16_t input_value = input_smem[reorder_idx];
      bf16_t weight_value = weight[reorder_idx];
      bf16_t normalized_value = Float2Bfloat16((float)input_value * (float)sumv);
      bf16_t weighted_value = Float2Bfloat16((float)weight_value * (float)normalized_value);
...
template void run_rmsnorm_x_bf16_nvfp4<16, 2048>(
  bf16_t*, bf16_t*, float, float, int, int16_t*, int16_t*, uint8_t*, uint8_t*,
  sf_t*, int, int
);
