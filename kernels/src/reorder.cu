...
__global__ void reorder_x_kernel(
  ...
  int16_t *reorder_base_index,
  uint8_t *reorder_is_contiguous,
  uint8_t *q_out,
  auto q_scale_tensor,
  int KQ, int KE
){
  ...
  // Reorder. The auto-rms order is stable-compacted for the ordinary region,
  // so many output groups still map to contiguous input segments.
  int reorder_frag[elements_per_thread];
  int reorder_offset = tid * GROUP_SIZE;
  int base_index = 0;
  bool contiguous_segment = false;
  if (reorder_base_index != nullptr && reorder_is_contiguous != nullptr && reorder_is_contiguous[tid] != 0) {
    base_index = reorder_base_index[tid];
    contiguous_segment = true;
  } else {
    base_index = reorder_index[reorder_offset];
    contiguous_segment = true;
    #pragma unroll
    for(int i = 0; i < elements_per_thread; ++i){
      reorder_frag[i] = reorder_index[reorder_offset + i];
      contiguous_segment = contiguous_segment && (reorder_frag[i] == base_index + i);
    }
  }
  if (contiguous_segment) {
    #pragma unroll 4
    for(int i = 0; i < elements_per_thread; ++i){
      input_frag[i] = input[base_index + i];
    }
  } else {
    #pragma unroll 4
    for(int i = 0;i < elements_per_thread;++i){
      input_frag[i] = input[reorder_frag[i]];
    }
  }
  // Reduce to get max
...
__global__ void reorder_w_kernel(
  ...
  // Reorder. Fast path contiguous segments from the stable-compacted region.
  int reorder_frag[elements_per_thread];
  int reorder_offset = tid * GROUP_SIZE;
  int base_index = 0;
  bool contiguous_segment = false;
...
__global__ void reorder32_x_kernel(
  ...
  // Reorder. Fast path contiguous segments from the stable-compacted region.
  int reorder_frag[ELEMENTS_PER_THREAD];
  int reorder_offset = tid * ELEMENTS_PER_THREAD;
  int base_index = 0;
  bool contiguous_segment = false;
  if (reorder_base_index != nullptr && reorder_is_contiguous != nullptr && reorder_is_contiguous[tid] != 0) {
    base_index = reorder_base_index[tid];
...
template void run_reorder_x_bf16_nvfp4<16, 2048>(
  bf16_t*, int, int16_t*, int16_t*, uint8_t*, uint8_t*,
  sf_t*, int, int
);
...
template void run_reorder_x_bf16_nvfp4<16, 1024>(
  bf16_t*, int, int16_t*, int16_t*, uint8_t*, uint8_t*,
  sf_t*, int, int
);

template void run_reorder_w_bf16_nvfp4<16, 1024>(
  bf16_t*, int, int16_t*, int16_t*, uint8_t*, uint8_t*,
  sf_t*, int, int
);

template void run_reorder_x_bf16_nvfp4<16, 2560>(
  bf16_t*, int, int16_t*, int16_t*, uint8_t*, uint8_t*,
  sf_t*, int, int
);

template void run_reorder_w_bf16_nvfp4<16, 2560>(
  bf16_t*, int, int16_t*, int16_t*, uint8_t*, uint8_t*,
  sf_t*, int, int
);
...
template void run_reorder_x_bf16_nvfp4<16, 6144>(
  bf16_t*, int, int16_t*, int16_t*, uint8_t*, uint8_t*,
  sf_t*, int, int
);

template void run_reorder_w_bf16_nvfp4<16, 6144>(
  bf16_t*, int, int16_t*, int16_t*, uint8_t*, uint8_t*,
  sf_t*, int, int
);

template void run_reorder_x_bf16_nvfp4<16, 9728>(
  bf16_t*, int, int16_t*, int16_t*, uint8_t*, uint8_t*,
  sf_t*, int, int
);

template void run_reorder_w_bf16_nvfp4<16, 9728>(
  bf16_t*, int, int16_t*, int16_t*, uint8_t*, uint8_t*,
  sf_t*, int, int
);
...
template void run_reorder32_x_bf16_nvfp4<32, 17408>(
  bf16_t*, int, int16_t*, int16_t*, uint8_t*, uint8_t*,
  sf_t*, int, int
);

template void run_reorder32_w_bf16_nvfp4<32, 17408>(
  bf16_t*, int, int16_t*, int16_t*, uint8_t*, uint8_t*,
  sf_t*, int, int
);
