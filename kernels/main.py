...
KERNEL_BUILD_DIR = Path(__file__).resolve().parent / "build"
...
for i in range(K // step + 1):
    ...
    A, SFA = ocgcuda.reorder_quantize_x(X / scale_x, reorder_index, KE)
    B, SFB = ocgcuda.reorder_quantize_w(W / scale_w, reorder_index, KE)
    torch.cuda.synchronize()

    C = ocgcuda.matmul(A, B, SFA, SFB, scale_x * scale_w)
    torch.cuda.synchronize()
